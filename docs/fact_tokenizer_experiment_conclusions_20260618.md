...
本文综合当前 FACT tokenizer MVP 的主要实验，从 debug smoke、500-take 基线、v4/v5 系列 finetune、4.0 非 multi-exo 改进实验，到最新 v5k 训练结果。结论只覆盖当前仓库的一阶段 tokenizer，不扩展到 WAM、Action Head、SONIC 或完整 robot deployment。
...
截至 v5k，当前 FACT tokenizer 一阶段还没有满足 go / no-go gate。最接近阶段性可用的不是最新 v5k，而是 v4d/v5d/v5f 这一带：它们 view leakage 较低，code usage 未完全塌，但仍没有过 take leakage、causality 和 usage gate。
...
### 4.3 Take leakage 是当前最大稳定瓶颈

所有正式 heldout gate 中，`tuple_take_nmi` 都明显高于 0.35：
...
这说明 token 仍然强烈绑定 take identity、场景上下文、任务片段或视频局部统计，而不是纯 action prototype。这个问题没有被更强的 4.0 teacher、usage regularization 或 anti-take loss 解决。
...
v5k 的 effective codes 只有 13.9。这说明当前 loss 堆叠方式可能把模型推向更保守的少数 prototype，而不是打开更多 action prototype。
...
### 4.6 当前 16 transitions/take 的数据采样可能是根因之一

当前每个 take 约 16 个 transition。这个设置容易导致：

- 每个 take 内 action coverage 太稀疏，token 更容易绑定 take-level context。
- same-take negative 不够丰富，模型难以区分同一 take 内的不同动作阶段。
- train/heldout 虽按 take 划分，但训练阶段每个 take 的局部多样性不足，anti-take loss 只能惩罚表层分布，不能提供足够 action phase variation。

因此，继续在同一 16 transitions/take 数据上单纯加 loss，边际收益已经很低。

## 5. 当前判断

当前最佳解释是：

FACT tokenizer 的工程路径已经跑通，4.0 中除 full multi-exo teacher 聚合以外的多数非 multi-exo 机制也已经实现并训练验证过；但是当前 token 还没有成为稳定的 action-centric shared token。主要障碍不是 view-specific leakage，而是 take/context leakage、codebook collapse 和 action causality 不足。

因此，当前阶段不建议进入 WAM 或 Action Head，也不建议立即加入完整 multi-exo teacher。应先修复数据采样与一阶段 tokenizer gate。
...
可视化结论与数值一致：训练 reconstruction MSE 有改善，但 codebook 使用不健康，v5k 不适合作为冻结 tokenizer。
